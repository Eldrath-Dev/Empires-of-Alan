...
public class TownDemoteCommand extends SubCommand {

    public TownDemoteCommand(EmpiresOfAlan plugin) {
        super(plugin);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        // Check if player has demote permission (Mayor+)
        ...
        if (townManager.demoteResident(player.getUniqueId(), targetPlayer.getUniqueId())) {
            // Get the new role
            TownRole newRole = targetResident.getTownRole();

            player.sendMessage(configManager.getMessage("towns.demoted",
            ...
            if (onlineTarget != null && onlineTarget.isOnline()) {
                onlineTarget.sendMessage(configManager.getMessage("towns.you-were-demoted",
                                "§aYou have been demoted to §e{0} §ain your town.")
                        .replace("{0}", newRole.getDisplayName()));
            }

            return true;
        } else {
            player.sendMessage(configManager.getMessage("towns.demotion-failed",
    ...
    public String getDescription() {
        return "Demote a player in your town";
    }

    @Override
    public String getUsage() {
        return "/town demote <player>";
    }

    @Override
    public String getPermission() {
        return "empiresofalan.town.demote";
    }
}